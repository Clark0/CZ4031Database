...
class Handler(handler.ContentHandler):
    def __init__(self, pub_file, pub_authors_file):
        ...
        self.fields = ['pubid', 'pubtype', 'mdate', 'pubkey', 'crossref', 'journal',
                       'year', 'month', 'title']
        self.pubId = 0
        self.attrs = {}
        self.authors = []
    ...
    def startDocument(self):
        ...
        self.pub_authors_writer.writerow(['pubid', 'authors'])

    def startElement(self, name, attrs):
        if name in self.subPublication:
            mdate, key = attrs.getValue('mdate'), attrs.getValue('key')
            self.pubId += 1
            self.attrs = {'pubid': self.pubId, 'pubtype': name, 'mdate': mdate, 'pubkey': key}
            self.authors = []
    ...
    def write_authors(self):
        row = [self.pubId, self.authors]
        self.pub_authors_writer.writerow(row)
...
if __name__ == '__main__':
    if not os.path.exists('out'):
        os.mkdir('out')
    pub_file, pub_authors_file, = os.path.join('out', 'pub.csv'), os.path.join('out', 'pub_authors.csv'),
    parser = make_parser()
